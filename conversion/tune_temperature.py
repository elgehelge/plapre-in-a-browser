"""Temperature-tuning endpoint: render a long, multi-sentence text through OUR
full ONNX pipeline at several sampling temperatures so you can pick the value
that sounds best, then lock it into the library (it's a runtime knob, NOT baked
into the artifacts — see docs/TUNING.md).

Mirrors the browser engine exactly: normalize, split into sentences, synthesize
each sentence independently (LM -> decoder -> vocoder), concatenate with a short
silence between them. Loads the 521 MB LM once and reuses it across every
sentence and temperature.

Output: golden/demo_t{TEMP}.wav (one per temperature), git-ignored.

  python tune_temperature.py [--text-file FILE] [--seed N]
                             [--temps 0.5,0.6,0.7] [--speaker tor]

Sentence splitting + normalization mirror plapre.inference (audited 1:1); the
sampler + per-sentence RNG reset mirror web/src/pipeline/{lm,sampling}.ts, so a
temperature that sounds good here is the same one to pass as
generation.temperature in the library.
"""
